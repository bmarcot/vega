...
static unsigned long timerid_bits = 0;
...
static int reserve_timer_id(timer_t *timerid)
{
	...
	bit = find_first_zero_bit(&timerid_bits, BITS_PER_LONG);
	if (bit == BITS_PER_LONG)
		return -1;
	bitmap_set_bit(&timerid_bits, bit);
	*timerid = bit;

	return 0;
}

static void release_timer_id(timer_t timerid)
{
	bitmap_clear_bit(&timerid_bits, timerid);
}
	...
	timer_t			*timerid)
{
	...
	if (reserve_timer_id(&pt->id)) {
		kmem_cache_free(posix_timer_cache, pt);
		return EAGAIN;
	}
	...
	timer_t			timerid)
{
	...
	hrtimer_cancel(&pt->timer);
	release_timer_id(pt->id);

	/*
	 * The treatment of any pending signal generated by the
	 * deleted timer is unspecified. Cancel any pending signal.
	 */
	remove_pending_signal(&pt->sigqueue, pt->task);
