...
void *malloc(size_t size)
{
	...
	list_for_each_entry(free_block, &blocks, list) {
		if (get_block_free(free_block) && (get_block_length(free_block) >= size)) {
			if ((get_block_length(free_block) - size) > sizeof(struct malloc_tag)) {
				set_block_length(free_block, get_block_length(free_block) - size);
				new_block = (struct malloc_tag *)((u32) free_block + get_block_length(free_block));
...
void free(void *ptr)
{
	...
	if (block->list.next != &blocks) {
		...
		if (get_block_free(next_block)) {
			set_block_length(block, get_block_length(block)
					+ get_block_length(next_block));
